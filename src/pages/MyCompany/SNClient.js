...
function SNClient({expanded}) {
  ...
  const initialState = [{plantId: '0000', key: companyKeys.SNCLIENT, code: '', description: '', length: ''}];
  ...
  useEffect(() => {
    if (expanded === 'panel12') getCompany();
  }, [expanded]);

  const getCompany = async () => {
    setLoading(true);
    const {data, success} = await GetCompany(companyKeys.SNCLIENT);
    if (success) {
        ...
        elements.push(
          <Card key={i} style={styles.checkboxCard}>
            <div style={styles.inputDiv}>
              <TextInput value={info[i].code} onChange={text => onInputChange('code', text, i)} label={'Client name'} width={9} />
              <TextInput
                value={info[i].description}
                onChange={text => onInputChange('description', text, i)}
                label={'Serial Number Expiry Time'}
                width={7}
              />
              <TextInput
                value={info[i].length}
                onChange={text => onInputChange('length', text, i)}
                label={'Purge method for used crypto codes'}
                width={9}
              />
