...
function TLCarrier({expanded}) {
  ...
  const initialState = [{plantId: '0000', key: companyKeys.TL_CARRIER, code: '', description: '', length: ''}];
  ...
  useEffect(() => {
    if (expanded === 'panel13') getCompany();
  }, [expanded]);

  const getCompany = async () => {
    setLoading(true);
    const {data, success} = await GetCompany(companyKeys.TL_CARRIER);
    if (success) {
        ...
        elements.push(
          <Card key={i} style={styles.checkboxCard}>
            <div style={styles.inputDiv}>
              <TextInput value={info[i].code} onChange={text => onInputChange('code', text, i)} label={'Material ID'} width={9} />
              <TextInput value={info[i].description} onChange={text => onInputChange('description', text, i)} label={'Carrier type'} width={7} />
              <TextInput value={info[i].length} onChange={text => onInputChange('length', text, i)} label={'Carrier ID'} width={9} />
